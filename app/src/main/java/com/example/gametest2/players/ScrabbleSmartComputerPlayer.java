            ...
            for (int k = 0; k < 7; k++) {
                for (int i = 0; i < Board.BOARD_SIZE; i++) {
                    for (int j = 0; j < Board.BOARD_SIZE; j++) {
                        ...
                        //go through each space that has at least one tile underneath
                        if (row != Board.BOARD_SIZE - 1 && !bd.boardTiles[row + 1][col].getEmpty() && bd.boardTiles[row][col].getEmpty()) {
                            if (col > 0 && bd.boardTiles[row][col - 1].getEmpty()) {
                                if (col < Board.BOARD_SIZE - 1 && bd.boardTiles[row][col + 1].getEmpty()) {
                                    ...
                                    while (tempRow != Board.BOARD_SIZE && !bd.boardTiles[tempRow][col].getEmpty()) {
                                        //go through all the way to the down to see if it is a word
                                        int tileScore = bd.boardTiles[tempRow][col].getPoints();

                                        //check for any specialties
                                        if (bd.squares[tempRow][col].getType() == Square.DL) {
                                            tileScore = tileScore * 2;
                                        }
                                        if (bd.squares[tempRow][col].getType() == Square.TL) {
                                            tileScore = tileScore * 3;
                                        }
                                        if (bd.squares[tempRow][col].getType() == Square.DW) {
                                            doubleWord = true;
                                        }
                                        if (bd.squares[tempRow][col].getType() == Square.TW) {
                                            tripleWord = true;
                                        }

                                        //set the score and the char
                                        score = score + tileScore;
                                        word = word + bd.boardTiles[tempRow][col].getChar();
                                        tempRow++;
                                    }
